...
from helper_functions.utils import (
    ...
    encode_labels,
    correct_permutation_test, repeated_cv_confidence_intervals, oversample_data,
    check_missing_values, train_val_test_split_exception
)
...
exceptions_data = datasets_merged['clonal_depth']['df']['Vaccinee']
exceptions_labels = datasets_merged['clonal_depth']['df']['response_label']

train_vaccinees, val_vaccinees, train_vaccinees_exceptions, val_vaccinees_exceptions, test_vaccinees = train_val_test_split_exception(
    vaccinees, abtiters['response_label'],exceptions_data, exceptions_labels , random_state=42
)
...
for oversample_method in OVERSAMPLING_METHODS:
    ...
    for dataset in datasets_merged:
        ...
        if dataset in ['clonal_depth', 'clonal_breadth']:
            train_df = curr_df[curr_df['Vaccinee'].isin(train_vaccinees_exceptions)]
            val_df = curr_df[curr_df['Vaccinee'].isin(val_vaccinees_exceptions)]
        ...
        if OVERSAMPLING_METHOD is not None:
            ...
            print(f"Class distribution for Val set: {Counter(y_val)}")
    ...
    def train_and_predict_best_model(X_train, y_train, X_val, y_val, X_test, y_test, data_name):
        ...
        best_model_training_metrics = None
        ...
        for name, model in models.items():
            ...
            validation_results.append({
                ...
                'CV Mean (Val Metric)': metrics.get('CV mean', None),
                'P-value': p_value
            })
            ...
            if current_composite_score > best_composite_score:
                ...
                elif abs(metrics.get('CV mean', -1) - best_model_training_metrics.get('CV mean', -2)) <= 0.001 and p_value < best_model_p_value:
                    best_composite_score = current_composite_score
                    best_model = clone(model)
                    best_model_name = name
                    best_model_training_metrics = metrics
