...
def plot_evaluation_results_by_smote(csv_file, output_dir):
    ...
    for method in smote_methods:
        ...
        if pd.isna(method) or method == "":
            df_method = df[df['Oversampling_Method'].isna() | (df['Oversampling_Method'] == "")]
        else:
        ...
        train_error_lower = df_method["Balanced_Accuracy"] - df_method["train IC lower"]
        train_error_lower = train_error_lower.clip(lower=0)  # Ensure no negative error
        train_error_upper = df_method["train IC upper"] - df_method["Balanced_Accuracy"]
        train_error_upper = train_error_upper.clip(lower=0)  # Ensure no negative error
        train_error = [train_error_lower, train_error_upper]

        test_error_lower = df_method["Test Balanced_Accuracy"] - df_method["test IC lower"]
        test_error_lower = test_error_lower.clip(lower=0)  # Ensure no negative error
        test_error_upper = df_method["test IC upper"] - df_method["Test Balanced_Accuracy"]
        test_error_upper = test_error_upper.clip(lower=0)  # Ensure no negative error
        test_error = [test_error_lower, test_error_upper]

        # Create the plot
        fig, ax = plt.subplots(figsize=(12, 8))
        ax.bar(x - width / 2, df_method["Balanced_Accuracy"], width,
                            yerr=train_error, capsize=5, label='Train Balanced Accuracy',
                            color='lightgreen', edgecolor='black')
...
csv_files = [
    "TEST_UNCOMPRESSED_clonal_breadth_data.csv",
    "TEST_UNCOMPRESSED_clonal_depth_data.csv",
    "TEST_UNCOMPRESSED_cytokines_data.csv",
    "TEST_UNCOMPRESSED_cytometry_data.csv",
    "TEST_UNCOMPRESSED_RNa_data_data.csv"
]

# Define the output directory where you want to save the plots
output_dir = "uncompressed_plots"
os.makedirs(output_dir, exist_ok=True)
